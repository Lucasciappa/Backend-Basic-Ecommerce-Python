...
import base64
class Usuario():
    def __init__(self,id,nombre,apellido,fecha_nac,interes,celular,email,password,genero,ciudad,banco):
        self.id=0
        self.nombre=nombre
...
# Registrar Usuarios
    def save(self):
        sql="INSERT INTO usuarios (nombre, apellido,fecha_nac, interes,celular,email,contraseña,genero,id_ciudad,id_banco) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        val=(self.get_nombre(),self.get_apellido(),self.get_fecha_nac(),self.get_interes(),self.get_celular(),self.get_email(),self.get_password(),self.get_genero(),self.get_ciudad(),self.get_banco())
